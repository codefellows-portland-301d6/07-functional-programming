...
(function(module) {
  ...
  Article.allAuthors = function() {
    ...
    /* TODO: For our `reduce` that we'll chain here -- since we are trying to
      ...
      .reduce(function(uniqueAuthorNames, name) {
        if (uniqueAuthorNames.indexOf(name) === -1) {
          uniqueAuthorNames.push(name);
        }
  ...
  Article.numWordsByAuthor = function() {
    ...
    return Article.allAuthors().map(function(author) {
      return {
        ...
        .map(function(curAuthArticle) {
          return curAuthArticle.body.match(/\w+/g).length;
        })
